...
public class UsuarioDAO {
    ...
    public boolean atualizarAdmin(String nome, String endereco, String email, String login, String senha) {
        boolean sucesso = false;
        try {
            Class.forName(JDBC_DRIVER);
            Connection c = DriverManager.getConnection(JDBC_URL, JDBC_USUARIO, JDBC_SENHA);
            PreparedStatement ps = c.prepareStatement("UPDATE usuario SET nome = ?, endereco = ?, email = ?, login = ?, senha = ?, administrador = true WHERE login = ?");
            ps.setString(1, nome);
            ps.setString(2, endereco);
            ps.setString(3, email);
            ps.setString(4, login);
