...
public class UsuarioDAO {
    ...
    public boolean atualizarCliente(String nome, String endereco, String email, String login, String senha, int id) {
        boolean sucesso = false;
        try {
            Class.forName(JDBC_DRIVER);
            Connection c = DriverManager.getConnection(JDBC_URL, JDBC_USUARIO, JDBC_SENHA);
            PreparedStatement ps = c.prepareStatement("UPDATE usuario SET nome = ?, endereco = ?, email = ?, login = ?, senha = ?, administrador = false WHERE id = ?");
            ps.setString(1, nome);
            ps.setString(2, endereco);
            ps.setString(3, email);
            ps.setString(4, login);
    ...
    public boolean atualizarAdmin(String nome, String endereco, String email, String login, String senha, int id) {
        boolean sucesso = false;
        try {
            Class.forName(JDBC_DRIVER);
            Connection c = DriverManager.getConnection(JDBC_URL, JDBC_USUARIO, JDBC_SENHA);
            PreparedStatement ps = c.prepareStatement("UPDATE usuario SET nome = ?, endereco = ?, email = ?, login = ?, senha = ?, administrador = true WHERE id = ?");
            ps.setString(1, nome);
            ps.setString(2, endereco);
            ps.setString(3, email);
            ps.setString(4, login);
